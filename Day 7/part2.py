class Node:
    ...
    def computeSize(self,tally, necessary) -> None:
        for key in self.children:
            if(self.children[key].getSize() >= necessary):
                tally.append(self.children[key].getSize())
            self.children[key].computeSize(tally, necessary)
...
def main():
    ...
    necessary = 30000000 - (70000000 - folders.getSize())

    tally = []
    folders.computeSize(tally, necessary)
