...
def explore(start : tuple, nodes, nodeMap, __n=(26,26), score=0, max_score=0):
    if((__n[0] <= 0 and __n[1] <= 0) or len(nodes)==0):
        return score
    remaining_score = sum([node.rate*(__n[0]-min(nodeMap[(start[0],node)], nodeMap[(start[1],node)])) for node in nodes])
    #remaining_score = max(__n)*sum([node.rate for node in nodes])
    if max_score-score > remaining_score:
        return score
    maxp = 0
    for node in sorted(nodes, reverse=True):
        for elenode in sorted(nodes-set([node]), reverse=True):
            tmp0 = max(__n[0]-nodeMap[(start[0],node)]-1, 0)
            tmp1 = max(__n[1]-nodeMap[(start[1],elenode)]-1, 0)
            newscore = score + tmp0*node.rate + tmp1*elenode.rate
            p = explore((node, elenode), nodes-set([node,elenode]), nodeMap, (tmp0, tmp1), newscore, max_score=max(maxp,max_score))
            maxp = max(p, maxp)
    return maxp
...
def main(filename):
    ...
    print(explore((start,start),validNodes,nodeMap))
